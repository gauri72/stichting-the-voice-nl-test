...
export async function createDiscount(req, res) {
  try {
    ...
    const existing = await DiscountCode.findOne({ code: uppercaseCode }).select("_id").lean();
    if (existing) {
      return res.status(400).json({ error: `Discount code '${uppercaseCode}' already exists.` });
    }
