...
/** Pre-fill Payment Element (incl. iDEAL full name) from the donor/sponsor details step. */
export function buildPaymentElementOptions(payer = {}) {
  const name = String(
...
export async function confirmCheckoutPayment(stripe, elements, { returnUrl, payer }) {
  const billingDetails = resolvePayerBillingDetails(payer);
