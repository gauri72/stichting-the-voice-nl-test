...
export default function CartDrawer({ cart, drawerOpen, closeDrawer, removeFromCart, updateQty, subtotalMinor, cashbackMinor }) {
  ...
  return (
    <>
      ...
      <div
        role="dialog"
        aria-label={t("vcommerceShop:cartDrawer.ariaLabel")}
        style={{
          position: "fixed",
          top: 0,
          right: 0,
      ...
      >
        ...
        <div style={{ padding: "20px 20px 16px", borderBottom: "1px solid var(--color-border,rgba(128,128,128,0.15))", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <h2 style={{ margin: 0, fontSize: "1.05rem", fontWeight: 700 }}>{t("vcommerceShop:cartDrawer.title")}</h2>
          ...
          <button type="button" onClick={closeDrawer}
            ...
            aria-label={t("vcommerceShop:cartDrawer.closeAriaLabel")}
          >✕</button>
        </div>
        ...
        <div style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: "16px 20px" }}>
          {items.length === 0 ? (
            <div style={{ textAlign: "center", padding: "60px 0", color: "var(--color-text-muted,#aaa)" }}>
              ...
              <p style={{ margin: 0 }}>{t("vcommerceShop:cartDrawer.empty")}</p>
            </div>
          ) : (
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <button type="button"
                        ...
                        {t("vcommerceShop:cartDrawer.remove")}
                      </button>
                    </div>
                  </div>
                  <div style={{ flexShrink: 0, textAlign: "right" }}>
                    <p style={{ margin: 0, fontWeight: 700, fontSize: "0.9rem" }}>{formatPrice(item.priceMinor * item.quantity, item.currency)}</p>
                    {item.quantity > 1 && <p style={{ margin: "2px 0 0", fontSize: "0.72rem", color: "var(--color-text-muted,#aaa)" }}>{t("vcommerceShop:cartDrawer.priceEach", { price: formatPrice(item.priceMinor, item.currency) })}</p>}
                  </div>
                </div>
              ))}
            </div>
        ...
        {items.length > 0 && (
          <div style={{ padding: "16px 20px 24px", borderTop: "1px solid var(--color-border,rgba(128,128,128,0.15))" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8, fontSize: "0.9rem" }}>
              <span style={{ color: "var(--color-text-secondary,#666)" }}>{t("vcommerceShop:cartDrawer.subtotal")}</span>
            ...
            {cashbackMinor > 0 && (
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16, fontSize: "0.85rem" }}>
                <span style={{ color: "var(--vco-accent-teal,#14B8A6)" }}>🎁 {t("vcommerceShop:cartDrawer.cashback")}</span>
            ...
            >
              {t("vcommerceShop:cartDrawer.checkout", { price: formatPrice(subtotalMinor, items[0]?.currency || "eur") })}
            </Link>
            <button type="button" onClick={closeDrawer}
              ...
              {t("vcommerceShop:cartDrawer.continueShopping")}
            </button>
          </div>
        )}
      </div>
