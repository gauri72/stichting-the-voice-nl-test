    ...
    <AdminLayout pageTitle="Finance Settings" pageSubtitle="Invoice defaults, payment details and organization information.">
      <div className="admin-finance">
        ...
        <form className="admin-finance__form-grid admin-finance__settings-form" onSubmit={handleSubmit}>
          ...
          <div className="admin-finance__field admin-finance__full">
            <label>Organization Name</label>
            <input value={form.organizationName} onChange={(e) => setForm((f) => ({ ...f, organizationName: e.target.value }))} disabled={!canWrite} />
          </div>
          <div className="admin-finance__field admin-finance__full">
            <label>Address</label>
            <textarea value={form.address} onChange={(e) => setForm((f) => ({ ...f, address: e.target.value }))} rows={2} disabled={!canWrite} />
          </div>
          ...
          <div className="admin-finance__field">
            ...
            <input value={form.vatNumber} onChange={(e) => setForm((f) => ({ ...f, vatNumber: e.target.value }))} disabled={!canWrite} />
          </div>
          ...
          <div className="admin-finance__field-row">
            <div className="admin-finance__field">
              ...
              <input value={form.invoicePrefix} onChange={(e) => setForm((f) => ({ ...f, invoicePrefix: e.target.value }))} disabled={!canWrite} />
            </div>
            <div className="admin-finance__field">
              <label>Default VAT %</label>
              <input type="number" value={form.defaultVatRate} onChange={(e) => setForm((f) => ({ ...f, defaultVatRate: Number(e.target.value) }))} disabled={!canWrite} />
            </div>
          </div>
          <div className="admin-finance__field">
            <label>Payment Terms (days)</label>
            <input type="number" value={form.paymentTermsDays} onChange={(e) => setForm((f) => ({ ...f, paymentTermsDays: Number(e.target.value) }))} disabled={!canWrite} />
          </div>
          <div className="admin-finance__field admin-finance__full">
            <label>Invoice Terms</label>
            <textarea value={form.invoiceTerms} onChange={(e) => setForm((f) => ({ ...f, invoiceTerms: e.target.value }))} rows={3} disabled={!canWrite} />
          </div>
          <div className="admin-finance__field admin-finance__full">
            <label>Footer Note</label>
            <textarea value={form.invoiceFooterNote} onChange={(e) => setForm((f) => ({ ...f, invoiceFooterNote: e.target.value }))} rows={2} disabled={!canWrite} />
          </div>
